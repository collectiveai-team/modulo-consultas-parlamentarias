...
class CSVDataPopulator:
    ...
    def _read_csv_safely(self, filepath: Path, separator: str = ";") -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully read {len(df)} rows from {filepath} with latin-1 encoding")
                return df
    ...
    def populate_asuntos_diputados(self) -> int:
        ...
        with Session(self.engine) as session:
            ...
            existing_ids = set(
                session.exec(select(DBAsuntoDiputado.asunto_id)).all()
            )
    ...
    def populate_bloques_diputados(self) -> int:
        ...
        with Session(self.engine) as session:
            existing_ids = set(
                session.exec(select(DBBloqueDiputado.bloque_id)).all()
            )
    ...
    def populate_legisladores_diputados(self) -> int:
        ...
        with Session(self.engine) as session:
            ...
            if new_records:
                session.add_all(new_records)
                session.commit()
                logger.info(f"Added {len(new_records)} new legisladores_diputados records")
            else:
        ...
        with Session(self.engine) as session:
            ...
            if new_records:
                ...
                for i in range(0, len(new_records), batch_size):
                    ...
                    logger.info(f"Added batch of {len(batch)} votaciones_diputados records")
                
                logger.info(f"Added {total_added} new votaciones_diputados records total")
                return total_added
            else:
